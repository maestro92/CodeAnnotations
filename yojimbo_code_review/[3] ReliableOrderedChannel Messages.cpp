...
now looks at how ReliableOrderedChannel sends out messages.
...
the default ConnectionConfig() just has one channel 
...
I assume that unless specified, that channel will be initlaized with CHANNEL_TYPE_RELIABLE_ORDERED, since in the enum, CHANNEL_TYPE_RELIABLE_ORDERED comes first 
...
2.  now we create the messages that the sender will send to the receiver 
-   as you can see here, we are just creating Block messages and we add a sequence number to it. 

-   we create some random data to fill up the blocks, then we attach the blocks to the message 

-   eventually we give it to the sender 
...
3.  let us look at what does the sender do with it in SendMessage();
    as you can see, we some differing logic depending if you are block message or not. 
    
    -   we first insert the message into to the ReliableOrderedChannel.m_messageSendQueue. then we fill up some infor the MessageSendQueueEntry.
        regardless if you are sending a regular message or a block message, we all put it in this m_messageSendQueue

    -   the entry->measuredBits will be explained later. 

    -   we also increase the m_sendMessageId for the counter.
...
4.  now lets go back to our tests
    let us look at the PumpConnectionUpdate(); function. that is where all the magic happens
...
5.  the PumpConnectionUpdate(); function is where Sender and receiver(); will try to send packets;
the idea is we call GeneratePacket(); and we pass in the variable packetData. The GeneratePacket(); will fill in information in the packetData variable.
